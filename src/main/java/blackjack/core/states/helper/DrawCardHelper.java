...
public class DrawCardHelper {
    public void playerDrawCard(DeckComponent playerDeckComponent, BlackjackCore core, int amount) {
        Card card = playerDeckComponent.drawCardToHand(amount).get(0);
        core.registerPlayerCardDraw(card);
        core.emitDrawCard();
    }

    public void enemyDrawCard(DeckComponent enemyDeckComponent, BlackjackCore core, int amount) {
